...
│       └── dmt143_plugin.py    # DMT143 露点变送器协议插件
├── main.py                      # 程序入口
...
4. 在发送区输入命令并发送（常用命令：
   ...
   - `HELP` - 显示命令列表
）
5. 接收区会自动解析并格式化显示 DMT143 的响应数据
6. 解析数据栏目显示详细的数据转换过程（包括字节解码、类型识别、数据提取等步骤）
7. 实时数据栏目动态更新露点、湿度等变量的最新值（自动进行华氏度/摄氏度转换）
